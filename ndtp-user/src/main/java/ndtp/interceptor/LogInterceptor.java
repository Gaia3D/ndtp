...
import ndtp.support.URLSupport;
...
public class LogInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
    	...
    	// 로그 예외 URL, 사용이력, 지도 map 은 너무 많아서 필터링
    	boolean isExceptionURI = false;
    	if(	( uri.indexOf("upload") < 0 && uri.indexOf("data") >= 0 && uri.indexOf("list") >= 0 )
    			|| ( uri.indexOf("data") >= 0 && uri.indexOf("map") >= 0 ) 
    			|| ( uri.indexOf("datas") >= 0 && uri.indexOf("list") >= 0 ) 
    			|| ( uri.indexOf("data-groups") >= 0) ) {
    		isExceptionURI = true;
    	}
    	if(!isExceptionURI) {
