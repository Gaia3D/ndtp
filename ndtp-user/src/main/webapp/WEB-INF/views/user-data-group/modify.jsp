...
<div id="wrap">
	...
	<div class="container" style="float:left; width: calc(100% - 78px);">
		...
		<div class="tabs" >
			<ul class="tab">
				<li onclick="location.href='/user-data-group/list'" class="on">데이터 그룹</li>
		...
		<div class="button-group">
			<div class="center-buttons">
				<input type="submit" value="<spring:message code='modified'/>" onclick="updateUserDataGroup();"/>
...
$(document).ready(function() {
	$("[name=basic]").filter("[value='${userDataGroup.basic}']").prop("checked",true);
	$("[name=available]").filter("[value='${userDataGroup.available}']").prop("checked",true);
});
...
// 수정
var updateUserDataGroupFlag = true;
function updateUserDataGroup() {
	if (validate() == false) {
		return false;
	}
	if(updateUserDataGroupFlag) {
		updateUserDataGroupFlag = false;
		var formData = $("#userDataGroup").serialize();		
		$.ajax({
			url: "/user-data-groups/${userDataGroup.userDataGroupId}",
			type: "POST",
